...
if ($pagina_id === 0 && $paginasResult->num_rows > 0) {
    ...
    // Resultset resetten zodat we later opnieuw kunnen loopen
    $paginasResult->data_seek(0);
}

// Hotspots ophalen voor de gekozen pagina
$hotspots = [];
if ($pagina_id > 0) {
    $stmt = $conn->prepare("
...
<main class="page">
    ...
    <form method="get" style="margin-bottom: 20px;">
        ...
        <select name="pagina_id" onchange="this.form.submit()">
            <?php while ($p = $paginasResult->fetch_assoc()): ?>
                <option value="<?= (int) $p['id'] ?>" <?= $pagina_id == $p['id'] ? 'selected' : '' ?>>
    ...
    <?php if ($pagina_id > 0): ?>
        <p>
            <a class="btn btn-primary" href="hotspots_edit.php?pagina_id=<?= $pagina_id ?>">
                + Nieuwe hotspot
            </a>
        </p>
